"""Conditional NIRS chain with ascending-row two-column accumulation.

Numerical baseline and physical assumptions remain unchanged. No clinical claim.
"""
import hashlib
import itertools
import json
...
import numpy as np
from bodytwin.geometry.optics.nirs_forward_v1 import absorption, sao2_hill, HB_G_DL, BLOOD_FRACTION, LAUNCHED
from bodytwin.chains.nirs_chain_v1 import NOMINAL, BANDS, SEED, DRAWS
...
def signal(paths,saturation,hb_g_dl=HB_G_DL,blood_fraction=BLOOD_FRACTION):
    ...
    weights=np.exp(-(paths@mua.T));total=np.add.accumulate(weights,axis=0)[-1];intensity=total/LAUNCHED
...
def main():
    ...
    (ROOT/'reports/nirs_reduced_chain_v1.json').write_text(json.dumps(report,indent=2)+'\n')
