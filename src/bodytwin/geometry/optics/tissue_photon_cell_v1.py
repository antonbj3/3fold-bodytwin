"""Exact open-cell rejection for certified integer-plane triangle meshes."""
import numpy as np
import warp as wp
from bodytwin.geometry.optics.tissue_photon_segmented_v1 import PACKET
from bodytwin.geometry.optics.tissue_photon_window2_v1 import _candidate_hit as window_hit
...
def _candidate_hit(mesh:wp.uint64,start:wp.vec3d,ray:wp.vec3d,limit:wp.float64):
    ...
    for axis in range(3):
        ...
        lower=wp.float64(float(wp.min(start[axis],end[axis])-pad))
        upper=wp.float64(float(wp.max(start[axis],end[axis])+pad))
        cell=wp.floor(lower)
        if not (lower>cell and upper<cell+wp.float64(1.0)):
            inside=0
    if inside==1:
        return int(-1)
    return window_hit(mesh,start,ray,limit)
...
class PreparedPhotonScene(PreparedBaseline):
    def __init__(self, vertices, faces, properties, face_front, face_back, *, device='cuda:0'):
        ...
        if v.ndim!=2 or v.shape[1]!=3 or f.ndim!=2 or f.shape[1]!=3 or len(f)==0:
            raise ValueError('Nonempty triangle arrays required')
        if not np.isfinite(v).all() or f.min()<0 or f.max()>=len(v):
            raise ValueError('Invalid geometry')
        a=v[f[:,0]];b=v[f[:,1]];c=v[f[:,2]]
        on_plane=(a==b)&(a==c)&(a==np.floor(a))
        if not np.all(np.any(on_plane,axis=1)):
            raise ValueError('Each triangle must lie on an integer axis plane')
        super().__init__(v,f,properties,face_front,face_back,device=device)
...
if __name__=='__main__':
    ...
    raise SystemExit(subprocess.call([sys.executable,str(root/'probes/optics/photon_segmented_transport_probe.py'),'--cell-certified',*sys.argv[1:]]))
