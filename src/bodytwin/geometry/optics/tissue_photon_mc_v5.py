"""Separate photon variant with BVH candidates and double-precision intersections.
...
def _candidate_hit(mesh:wp.uint64,start:wp.vec3d,ray:wp.vec3d,limit:wp.float64):
    ...
    for axis in range(3):
        extent=wp.max(wp.abs(start[axis]),wp.abs(end[axis]))+wp.float64(1.0)
        pad=extent*wp.float64(4.76837158203125e-7)
        lower[axis]=float(wp.min(start[axis],end[axis])-pad)
        upper[axis]=float(wp.max(start[axis],end[axis])+pad)
    ...
    while wp.mesh_query_aabb_next(candidates,face):
        a=wp.vec3d(wp.mesh_get_point(mesh,wp.mesh_get_index(mesh,3*face)))
        b=wp.vec3d(wp.mesh_get_point(mesh,wp.mesh_get_index(mesh,3*face+1)))
        c=wp.vec3d(wp.mesh_get_point(mesh,wp.mesh_get_index(mesh,3*face+2)))
...
if __name__=='__main__':
    ...
    raise SystemExit(subprocess.call([sys.executable,str(root/'probes/optics/photon_boundary_v5_probe.py')]))
