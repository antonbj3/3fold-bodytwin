"""Separate photon variant with cumulative weight quantization and normal offsets.
...
def _trace(mesh:wp.uint64,front:wp.array(dtype=int),back:wp.array(dtype=int),
    ...
    while weight>wp.int64(0) and events<50000 and leak==0:
        ...
        if hit==1:
            ...
            if transmit==0:p-=normal*1.0e-5
            else:p+=normal*1.0e-5
        else:
...
if __name__=='__main__':
    ...
    raise SystemExit(subprocess.call([sys.executable,str(root/'probes/optics/photon_boundary_v4_probe.py')]))
