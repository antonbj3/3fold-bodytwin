"""Transport with segmented candidate boxes and unchanged photon arithmetic.
...
@wp.func
def _window_hit(mesh:wp.uint64,start:wp.vec3d,ray:wp.vec3d,lower_t:wp.float64,limit:wp.float64):
    begin=start+ray*lower_t
    end=start+ray*limit
    lower=wp.vec3(0.0);upper=wp.vec3(0.0)
    for axis in range(3):
        extent=wp.max(wp.abs(begin[axis]),wp.abs(end[axis]))+wp.float64(1.0)
        pad=extent*wp.float64(4.76837158203125e-7)
        lower[axis]=float(wp.min(begin[axis],end[axis])-pad)
        upper[axis]=float(wp.max(begin[axis],end[axis])+pad)
    candidates=wp.mesh_query_aabb(mesh,lower,upper)
    face=int(0);best_face=int(-1);best=limit
    while wp.mesh_query_aabb_next(candidates,face):
        a=wp.vec3d(wp.mesh_get_point(mesh,3*face))
        b=wp.vec3d(wp.mesh_get_point(mesh,3*face+1))
        c=wp.vec3d(wp.mesh_get_point(mesh,3*face+2))
        e1=b-a;e2=c-a;h=wp.cross(ray,e2);det=wp.dot(e1,h)
        if det!=wp.float64(0.0):
            q=start-a;u=wp.dot(q,h)/det
            if u>=wp.float64(0.0) and u<=wp.float64(1.0):
                k=wp.cross(q,e1);v=wp.dot(ray,k)/det
                if v>=wp.float64(0.0) and u+v<=wp.float64(1.0):
                    t=wp.dot(e2,k)/det
                    if t>=wp.float64(0.0) and t<=best:
                        if t<best or best_face<0 or face<best_face:
                            best=t;best_face=face
    return best_face


@wp.func
def _candidate_hit(mesh:wp.uint64,start:wp.vec3d,ray:wp.vec3d,limit:wp.float64):
    if limit<=wp.float64(8.0):
        return _window_hit(mesh,start,ray,wp.float64(0.0),limit)
    lower=wp.float64(0.0)
    for step in range(128):
        upper=wp.min(limit,wp.float64(step+1)*wp.float64(8.0))
...
def simulate(vertices,faces,properties,face_front,face_back,*,source,direction,initial_region,
    ...
    paths=wp.zeros((photons,len(prop)-1),dtype=wp.float64,device=device)
...
if __name__=='__main__':
    ...
    raise SystemExit(subprocess.call([sys.executable,str(root/'probes/optics/photon_segmented_transport_probe.py'),*sys.argv[1:]]))
