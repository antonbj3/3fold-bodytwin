"""Acceptance tests for the shared preflight's second consumer: cardiac_output.

The cardiac cell reads the SAME nine quantities from the SAME
``<BODYTWIN_OUT>/metabolic_cost/metabolic_cost_results.json`` as thermoregulation, so the
metabolic-cost fixtures are reused byte-for-byte (no duplicates). The tests mirror the
thermoregulation subprocess tests:
  ...
  (b) ``tests/fixtures/i1/variants/wrong_unit`` -> exit 2, ``cardiac_output_refused.json`` with a
      ``unit mismatch`` and NO ``cardiac_output_results.json``;
  ...
  (d') ``tests/fixtures/i1/contract_gates_pass`` (contract-test fixture, not evidence) -> the
      ...
      cardiac cell's OWN pipeline gates are independent of the preflight and two of them
      (``forced_adversary_avo2_widening_necessary``, ``three_mechanism_corroboration``) FAIL on
      the adapted public payload, so the process exits 2 while still writing its report. This test
      therefore asserts the preflight state from the written report and documents the exit-2
      outcome explicitly (it does NOT pretend the cell's own gates pass);
  (e) a ``tc``-style legacy env run (cwd = cell dir, no PYTHONPATH, no registry) on the synthetic
      fixture -> exit 0, banner + ``synthetic_demo`` report;
  ...
  (g) a REFUSE removes a prior accepted ``cardiac_output_results.json`` (same hardening as thermo).
...
CARDIAC = SRC / "bodytwin" / "cells" / "cardiovascular" / "cardiac_output.py"
...
def _run_consumer(out_root, evidence_mode=None, use_registry=True):
    """Run the patched cardiac cell standalone (PYTHONPATH=src) against ``out_root``.
...
def _report_path(out_root):
    return out_root / "cardiac_output" / "cardiac_output_results.json"


def _refused_path(out_root):
    return out_root / "cardiac_output" / "cardiac_output_refused.json"
...
def test_consumer_wrong_unit_writes_refusal_and_exits_2(tmp_path):
    ...
    assert not _report_path(out).exists(), "REFUSE must not leave cardiac_output_results.json"
...
def test_consumer_contract_fixture_preflight_state_from_report(tmp_path):
    ...
    outcome) as scientific, but the cardiac cell's OWN
    gates do NOT all pass on the adapted public payload.

    Honest, explicit outcome: the cell writes ``cardiac_output_results.json`` with
    ``preflight.state == "ACCEPT_SCIENTIFIC"`` / ``evidence_class == "calibrated_public_model"``
    AND ``overall_pass is False`` (two independent cardiac gates fail), so the process exits 2.
    This test asserts the preflight state from the report (as the task directs) and pins the
    independent-gate outcome so the exit-2 is not mistaken for a preflight refusal.
    """
    ...
    # Independent cardiac gates fail on this payload -- documented, not hidden.
...
def test_refuse_removes_stale_accepted_output(tmp_path):
    """A later REFUSE must not leave a prior accepted cardiac_output_results.json for readers."""
    out = _copy_fixture(FIXTURES / "synthetic_demo", tmp_path)
    ...
    assert not _report_path(out).exists(), "REFUSE left a stale accepted cardiac_output_results.json"
...
def test_legacy_test_cells_env_runs_synthetic_as_demo(tmp_path):
    ...
    env["OMP_NUM_THREADS"] = "1"
    proc = subprocess.run(
        [sys.executable, CARDIAC.name],
        cwd=str(CARDIAC.parent),
        env=env,
        capture_output=True,
        text=True,
        timeout=900,
...
def test_generic_preflight_and_wrapper_agree_and_spec_alias_is_shared(tmp_path):
    ...
    and the exported metabolic-cost spec alias names the same object."""
...
def test_generic_preflight_refuses_wrong_unit_fixture():
    """The generic entry point applies the same shared contract to the same bad fixture."""
