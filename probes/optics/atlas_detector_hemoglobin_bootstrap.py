"""Paired packet bootstrap for fixed three-parameter detector information gain."""
...
def main():
    ...
    blood=mu-BACKGROUND[None,:]
    dh=-(paths@blood.T)*weights
    features=np.stack((masks[:,:,None]*weights[:,None,:],masks[:,:,None]*derivative[:,None,:],masks[:,:,None]*dh[:,None,:]),axis=-1)
    ...
    for leg in (0,1):
        ...
        moments=(counts[:,:k]@features[active].reshape(k,-1)).reshape(2000,6,2,3)
        signal=moments[:,:,:,0];gradient=moments[:,:,:,1]
        if np.any(signal<=0):raise ValueError('Bootstrap produced zero detector signal')
        J=np.stack((gradient/np.sqrt(signal),np.sqrt(signal),moments[:,:,:,2]/np.sqrt(signal)),axis=-1)
        selected=np.linalg.svd(J[:,:3].reshape(2000,6,3),compute_uv=False)[:,-1]
        reference=np.linalg.svd(J[:,3:].reshape(2000,6,3),compute_uv=False)[:,-1]
        ...
        rows.append(dict(percentiles=np.percentile(gain,[2.5,50,97.5]).tolist(),fraction_not_above_one=float(np.mean(gain<=1)),
                         ...
                         counts_hash=hashlib.sha256(counts.tobytes()).hexdigest(),finite_positive=bool(np.isfinite(gain).all() and np.isfinite(selected).all() and np.isfinite(reference).all() and min(np.min(gain),np.min(selected),np.min(reference))>0)))
    ...
    r=dict(parameter_order=['saturation','log_gain','log_hemoglobin'],parameter_scales=[1,1,1],rows=rows,gates=gates,draws=2000,seed=20260914,packet_histories=n,active_histories=k,
