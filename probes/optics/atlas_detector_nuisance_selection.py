"""Select on training packets with explicit independent detector gain nuisance."""
import argparse
import hashlib
import itertools
import json
...
def profile(matrix, triple):
    shared = matrix[list(triple)].reshape(6, 3)
    expanded = np.zeros((6, 5))
    expanded[:, :2] = shared[:, [0, 2]]
    for i in range(3):
        expanded[2*i:2*i+2, 2+i] = shared[2*i:2*i+2, 1]
...
def main():
    ...
    result = dict(rows=rows,gates={k:bool(v) for k,v in gates.items()},
                  ...
                  scope='Training-only nuisance-aware selection, independent packet evaluation; wavelength-shared detector gains, no clinical or sampling certificate.')
