"""Measure unknown common wavelength response after detector-gain profiling."""
import argparse
import hashlib
import json
...
def main():
    ...
    with np.load(a.arrays,allow_pickle=False) as source:
        for leg in (0,1):
            row=[]
            for identity in ('579','2610','048'):
                original=source[f'three_{identity}_jacobian_{leg}']
            ...
            rows.append(dict(triples=row,placement_gain=row[0]['after']/row[2]['after']))
