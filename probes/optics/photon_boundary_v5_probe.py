"""Unchanged physical boundary/slab gates for the separate double-intersection variant."""
import hashlib
import json
...
from bodytwin.geometry.optics.tissue_photon_mc_v5 import simulate,PACKET
...
def main():
    ...
    for case in ('cube60','cube60b','slab'):
        ...
        for _ in range(2):
            ...
            if case=='cube60':
                raw=np.load(ROOT/'reports/mcx_reference_fields_l4.npz')
                refs=[raw[key].astype(float).reshape(side,side,side)*5e-9 for key in ('first','second')]
            else:
    ...
    (ROOT/'reports/photon_boundary_v5.json').write_text(json.dumps(report,indent=2)+'\n')
