"""Observational copy of the failed transport; distinguish failure sites.
...
def _trace(mesh:wp.uint64,front:wp.array(dtype=int),back:wp.array(dtype=int),
    ...
    while weight>wp.int64(0) and events<50000 and leak==0:
        ...
        if distance<=0.0 or distance>1.0e19:leak=2
        if leak!=0:break
        ...
        if remaining>0.0 and weight>wp.int64(0) and leak==0:leak=4
    ...
    terminal[tid,4]=wp.int64(hit);terminal[tid,5]=wp.int64(remaining*1.0e9)
...
def simulate(vertices,faces,properties,face_front,face_back,*,source,direction,initial_region,
    ...
    terminal=wp.zeros((photons,6),dtype=wp.int64,device=device);counters=wp.zeros(3,dtype=wp.int64,device=device)
