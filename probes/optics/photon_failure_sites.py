...
from tissue_photon_failure_probe import simulate
...
def main():
    ...
    base=json.loads((ROOT/'reports/photon_cube_l4.json').read_text())['legs'][0]['hashes']
    rows=[]
    for _ in range(2):
        ...
        for code in range(1,5):
            selected=terminal[:,3]==code
            detail.append({'reason':code,'photons':int(selected.sum()),
                           'known_boundary_hits':int(terminal[selected,4].sum()),
                           'remaining_segment_max_mm':float(terminal[selected,5].max()/1e9) if selected.any() else 0.,
                           'remaining_segment_median_mm':float(np.median(terminal[selected,5])/1e9) if selected.any() else 0.})
    ...
    report={'legs':rows,'gates':gates,'reason_map':{'1':'region_label','2':'ray_distance','3':'grid_exit','4':'segment_limit'}}
    (ROOT/'reports/photon_failure_sites.json').write_text(json.dumps(report,indent=2)+'\n')
