"""Frozen placement bootstrap including two unknown common spectral gains."""
import argparse
import hashlib
import json
...
def extended(matrix):
    result=np.zeros((9,7));result[:,:5]=matrix
    signal=matrix[:,2:].sum(axis=1)
    result[1::3,5]=signal[1::3];result[2::3,6]=signal[2::3]
    return result
...
def sensitivities(moments):
    ...
    root=np.sqrt(signal);s=moments[...,1]/root;h=moments[...,2]/root
    out=[]
    for start in (0,3):
        nuisance=np.zeros((len(s),9,6));nuisance[:,:,0]=h[:,start:start+3].reshape(len(s),9)
        for detector in range(3):
            nuisance[:,3*detector:3*detector+3,detector+1]=root[:,start+detector]
        nuisance[:,1::3,4]=root[:,start:start+3,1]
        nuisance[:,2::3,5]=root[:,start:start+3,2]
        u,sigma,_=np.linalg.svd(nuisance,full_matrices=False)
        if np.any(sigma[:,-1]<=np.finfo(float).eps*9*sigma[:,0]):
            raise ValueError('Unresolved nuisance rank')
        interest=s[:,start:start+3].reshape(len(s),9)
        projected=interest-np.einsum('bij,bj->bi',u,np.einsum('bij,bi->bj',u,interest))
...
def main():
    ...
    qr=np.array([np.linalg.norm(residual(extended(matrix(paths,masks,[0,1,2],t)))) for t in ((4,7,9),(0,4,8))])
    ...
    result=dict(rows=rows,gates={key:bool(v) for key,v in gates.items()},draws=2000,seed=20260914,
                ...
                scope='Paired empirical bootstrap of frozen three-band placement, conditional optical model with detector and common spectral gains; no certified coverage or clinical precision.')
