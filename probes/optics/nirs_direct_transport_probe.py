...
def main():
    wp.init();m=layered_box();paths=np.load(ROOT/'reports/photon_detector_paths_l4.npz')['paths_0'];cases=[];ratios=[]
    for po2 in (40.,60.,80.,100.):
        ...
        for wavelength,row in zip((760,850),mua):
            ...
            for _ in range(2):
                out=simulate(m['vertices'],m['faces'],prop,m['front'],m['back'],source=[29,29,.0001],direction=[0,0,1],
                    initial_region=1,origin=[0,0,0],shape=[60]*3,reflect=True)
                escaped=out['terminal'][:,1]>0
                selected=escaped&(out['exits'][:,6]<=5.)&(np.sum((out['exits'][:,:3]-[39,29,0])**2,axis=1)<=4.)
    ...
    gates={'all_full_arrays_repeat_exact':all(c['legs'][0]==c['legs'][1] for c in cases),
        ...
        'path_prediction_integer_bound':all(r['prediction_error_integer']<=4119 for c in cases for r in c['legs']),
        'ratio_increases':all(a['ratio_760_850']<b['ratio_760_850'] for a,b in zip(ratios,ratios[1:]))}
    report={'cases':cases,'ratios':ratios,'gates':gates,'integer_bound':4119,'scope':'synthetic fixed-scattering wavelength transport; clinical slope calibration remains unverified'}
    (ROOT/'reports/nirs_direct_transport.json').write_text(json.dumps(report,indent=2)+'\n')
