"""Represented-input observer of frozen internal-region propagation."""
import hashlib
import json
...
def main():
    ...
    for mu in (0.,.2):
        ...
        prediction=np.floor(PACKET*np.exp(-float(np.float32(mu))*out['paths'][:,0])+.5).astype(np.int64)
    ...
    reference=json.loads((ROOT/'reports/photon_internal_air_l4.json').read_text())
    gates['unchanged_transport']=all(a['hashes']==b['hashes'] for a,b in zip(rows,reference['rows']))
    report=dict(rows=rows,gates=gates,scope='Synthetic ballistic internal-air control, not anatomical transport validation.')
    (ROOT/'reports/photon_internal_air_contract.json').write_text(json.dumps(report,indent=2)+'\n')
